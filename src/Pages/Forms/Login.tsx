...
export default function Login() {
  ...
  const handleSendCode = async () => {
    ...
    setLoading(true);
    try {
      await axios.post("http://localhost:8000/users/send-code", { email: signupData.email });
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post("http://localhost:8000/users", signupData);
      localStorage.setItem("token", response.data.token);
      ...
      <div className="w-full max-w-md rounded-md p-8 text-gray-300">
        ...
        <div className="flex border-b border-gray-700 mb-6">
          <button
            className={`flex-1 py-2 text-center text-lg font-semibold ${
              activeTab === "login" ? "border-b-2 border-blue-500 text-white" : "text-gray-400"
            }`}
          ...
          <button
            className={`flex-1 py-2 text-center text-lg font-semibold ${
              activeTab === "signup" ? "border-b-2 border-blue-500 text-white" : "text-gray-400"
            }`}
        ...
        {activeTab === "signup" && (
          <form className="space-y-6" onSubmit={handleSignupSubmit}>
            ...
            {showVerification && (
              <div>
                <label htmlFor="verification-code" className="block mb-2 text-sm text-gray-400">
                  Verification Code
                </label>
                <div className="flex space-x-2">
                  <input
